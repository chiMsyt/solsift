...
jobsift removes what needs no judgment; it does not rank, score or recommend.
...
def markdown(result: RunResult, profile: Profile) -> str:
    ...
    lines = [
        ...
        "",
        "> jobsift applied the disqualifying rules only. **It has not ranked "
    ...
    lines += [f"*Generated by jobsift at {datetime.now():%Y-%m-%d %H:%M}. "
              f"Re-run the rules for free with `jobsift rescreen`.*", ""]
    return "\n".join(lines)
